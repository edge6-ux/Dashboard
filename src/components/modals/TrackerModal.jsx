...
export default function TrackerModal({ ctx }) {
  const { setTrackerModalOpen, editingTrackerId, setEditingTrackerId, toast, user } = ctx
  ...
  useEffect(() => {
    if (editingTrackerId) {
      ...
      setName(''); setKeywords(''); setColor('#b9a9ff')
    }
  }, [editingTrackerId])

  const close = () => { setTrackerModalOpen(false); setEditingTrackerId(null) }

  const save = async () => {
    if (!name.trim()) { toast('Enter a tracker name', 'error'); return }
    if (!keywords.trim()) { toast('Enter at least one keyword', 'error'); return }
